...
#import ttk module koji sadrži notebook widget                     
from tkinter import ttk
#import svega iz tkinter modula, StringVar je u tom modulu
from tkinter import *
#import openpyxl modula za interakciju sa excel tablicama i sqlite3 za bazu podataka
import openpyxl, sqlite3


#Funkcija koja sadrži sve vezano za automatski unos podataka
def unos_podataka(event):
    #uzima putanju do excel datoteke iz Entry widgeta
    putanja_text = putanja_automatski_unos.get()
    #openpyxl pristupa podacima iz excel tablice
    workbook = openpyxl.load_workbook(putanja_text)
    #radimo sa sheet koji je aktivan, to jest koji je glavni kada se otvori excel tablica
    sheet = workbook.active
    
    #čitanje vrijednosti direktno iz excel tablice
    #lijeva strana excel tablice
    #definiramo Label, tab u kojem se nalazi, text labela, te dodajemo vrijednost iz excela, sa grid pozicioniramo na frame, sticky za uredniji izgled
    ...
    #datum i djelatnik SRP-a koji je radove obavio
    ...
    #spajamo se na bazu podataka sa datom putanjom gdje se nalazi, ili stvorimo novu na toj lokaciji ako ne postoji
    conn = sqlite3.connect('/home/magni/zavrsni_rad/izvjestaji_baza.db')
    #cursor je potreban za pretraživanje podataka, bez njega se može samo upisivati u bazu, ali kada bazu pretražujemo potreban je cursor
    c = conn.cursor()

    #stvaramo tablicu ako već ne postoji, sa unosima koji su nam potrebni
    # moramo definirati i vrstu podatka koji će biti unešen u koje polje
    # SQL naredbe se pišu velikim slovima, nazivi varijabli, stupaca i ostalog mogu biti proizvoljni 
    ...
    #nakon što smo tablicu stvorili, definiramo što točno unosimo u nju
    #oblik definicije (tablica(stupac),?)
    #u ovom slučaju npr (izvještaj_tablica(djelatnik),?),sheet.cell(row=5, column=3).value kako bi direktno iz excela povukao vrijednost
    def unesi_podatke_automatski():
        ...
        #potvrdi promjene i zatvori konekciju sa bazom podataka                
        conn.commit()

    #funkcija koja poziva sve prijašnje funkcije pritiskom na gumb "Pohrani u BP!"
    def automatski_upis(event):
        stvori_tablicu_automatski()
        unesi_podatke_automatski()
        #makni gumb nakon pritiska
        pohrani_podatke_automatski.destroy()
        #ispiši novi label umjesto gumba, kako bi korisnik znao da je odrađeno
        gotovo_automatski_label = Label(tab1, text="Pohranjeno!").grid(row=33, column=0)
        
    #gumb za spremanje podataka u bp, za izvršenje svih definiranih funkcija
...
def rucni_unos(event):
    #definicija varijabli za ručni unos
    ...
    #ukloni gumb nakon pritiska
    rucni_unos_button.destroy()

    #tab2, label i entry za osnovne informacije
    #textvariable uzima gore definirane varijable kako bi unjeo tekst koji upišemo u entry widget
    #prilikom upisa u bp metoda get() uzima unešenu vrijednost varijable
    ...
    #uspostava veze sa bazom podataka
    conn = sqlite3.connect('/home/magni/zavrsni_rad/izvjestaji_baza.db')
    #cursor potreban za pregled podataka u bazi
    c = conn.cursor()

    #stvori tablicu ako ne postoji, koristi ju ako postoji
    #jednaki nazivi stupaca kao i za automatski unos
    ...
    #upis podataka, ovaj put metodom get() dobivano vrijednost, za razliku od automatskog gdje smo koristili openpyxl
    def unesi_podatke_rucno():
        ...
        #pohrani podatke i prekini vezu s bazom                
        conn.commit()
    
    #izvedi sve definirane funkcije vezane za ručni unos
    def rucni_upis(event):
        stvori_tablicu_rucno()
        unesi_podatke_rucno()
        #ukloni gumb nakon pritiska
        pohrani_podatke_rucno.destroy()
        #ispiši poruku da je unos obavljen
        gotovo_rucno_label = Label(tab2, text="Pohranjeno!").grid(row=28, column=1)

    #gumb za pohranu u bp za ručni unos
...
#stvori roditelj direktorij  
#Tk() se koristi za stvaranje roditelj direktorija. Tk klasa se instancira bez ikakvih argumenata
root = tk.Tk() 
root.geometry("800x650")
#dodaj naslov za roditelj direktorij(root)
root.title("FERIT SRP") 
#stvori tab kontrolu
#opcije: Notebook() metoda prihvaća height, padding i width
#stvaranje tabova
#master: tab_control je roditelj widget za tabove
#opcije: Frame() metoda prihvaća class__, cursor, padding, relief, style, takefocus, height i width.
#objašnjenje: Frame widget se ponaša kao spremnik i koristi se za grupiranje ostalih widgeta zajedno 
...
#tab1 i tab2 su djeca widgeti tab_control
#opcije: add() metoda prihvaća sticky, state, padding, text, image, compound, underline
#objašnjenje: add() metoda se nalazi u tk.ttk.Notebook klasi, koristi se za dodavanje novih tabova u Notebook widget  
tab_control.add(tab1, text ='Automatski unos') 
tab_control.add(tab2, text ='Ručni unos')

#expand: osigurava jednaku raspodjelu prostora među viđetima koji imaju vrijednost proširenja različitu od nule 
#kada se roditelj widget proširi
#fill: fill opcija osigurava da widget zauzima prostor koji mu je dodijeljen 
#fill=”both” specificira da widget zauzima prostor i po X i po Y osi 
#fill=”X” specificira da widget zauzima prostor po X osi  
#fill=”Y” specificira da widget zauzima prostor po Y osi
#pack() metoda se koristi kako bi se widgeti organizirali u blokove prije nego se stave u roditelj widget
tab_control.pack(expand=1, fill="both") 

#tab1
#unosimo punu putanju do excel datoteke koju trebamo pohraniti u bp, spremamo unos u varijablu u obliku StringVar()
putanja_automatski_unos = StringVar() 
#objašnjenje: grid() metoda se koristi kako bi se roditelj widget razdvojio na redove i stupce
lokacija_entry = Entry(tab1, textvariable=putanja_automatski_unos, width=50).grid(row=0, column=1)

#gumb za učitavanje vrijednosti iz excel dokumenta nakon što smo unjeli putanju do njega
...
#tab2
#gumb koji će pozvati funkcije za ručni unos podataka u bp
...
#pokrećemo program
#mainloop() se ponaša kao beskonačna petlja i koristi se za pokretanje programa
root.mainloop()   